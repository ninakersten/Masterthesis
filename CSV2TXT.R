...
### Creating the 1. Stimuli-Dataset: "FGF1"###
  ...
  newdata2 <- subset(newdata, V4 == "0min" | V3 == "Insulin") # Filter for all "0min" and "FGF1" entries
  ...
  newdata2$V1 <- NULL
	...
	split_newdata4 <- newdata4[,1:20]	# Hälfte der Tabelle um mit kleineren Datensatz zu üben.
  ...
  # Replace ID by "A-Z" in the first row of the data frame
	...
	write.table(a, file = "CSV2TXTOutput.txt", quote = FALSE, row.names=FALSE, col.names = FALSE)
	newdata3 <- read.table(file="CSV2TXTOutput.txt", fill= TRUE, header = F)
	
	# add "NON" in first entry to get the same dimension as split_newdata5
  #x <- t(newdata3)
	# transpose the list and convert it into a dataframe 
  converted_data <- data.frame(as.list(t(newdata3)))
