...
for input in ./bnet_data_small_example/Network_*.bnet; do
...
for input2 in ./PyBoolNet/bnet_data_small_example/Network_*.bnet; do
...
python3 create_continuous_data.py $input2 $input2 [0,50] $1 [1,5] [0.4,0.6] [1,3]
...
before=$(date +%s)
python BinInfer.py input=$ts2b_input bin-method=$2 learn-method=$3 maxscore=10.0 solutions=3
after=$(date +%s)
...
python3 scoring_small_example.py $1 $2 $3
...
mkdir prediction_insilico inference_output_insilico goldstandard_insilico CSV_insilico_2_TXT CSV_insilico bnet_data_small_example
...
for newdir7 in ./bnet_data_small_example/*.bnet;do
	cp -f -i $newdir7 ./Backup/$newdir/bnet_data_small_example
done
